...
void triggerFromDTableToTex(std::string fileName)
{
    ...
    for (int i = 0; i < triggerFromDTable.size(); i++)
        texFile << "c|";
    texFile << "}" << endl;
    ...
    for (int i = 1; i < triggerInputsString.size(); i++)
        texFile << " & " << toTexTableCell(triggerInputsString[i]);
    texFile << "\\\\" << "\\hline" << endl;

    for (int i = 0; i < triggerFromDTable.size(); i++)
    {
        for (int j = 0; j < triggerFromDTable[i].size(); j++)
        {
            if (j == 0)
...
{
    vector<int> nonterminalLetterNumber(inputNonterminalDict.size(), 0);
    for (int i = 0; i < inputRules.size(); i++)
    {
        ...
        else
        {
            ...
            for (int j = 0; j < inputRules[i].condition.size()-1; j++)
            {
    ...
    for (int i = 0; i < nonterminalLetterNumber.size(); i++)
    {
        outputNonterminalDict.push_back(inputNonterminalDict[i]);
        for (int j = 0; j < nonterminalLetterNumber[i]; j++)
...
void graph::toDotFile(std::string fileName)
{
    ...
    for (int i = 0; i < connectionsToDraw.size(); i++)
    {
        for (int j = i + 1; j < connectionsToDraw.size(); j++)
        {
    ...
    for (int i = 0; i < connectionsToDraw.size(); i++)
    {
        for (int j = 1; j < connectionsToDraw[i].condition.size(); j += 2)
        {
    ...
    for (int i = 0; i < connectionsToDraw.size(); i++)
    {
        ...
        for (int j = 0; j < connectionsToDraw[i].condition.size(); j++)
...
void graph::toCubeDotFile(std::string fileName)
{
    ...
    for (int i = 0; i < nodesNames.size(); i++)
    {
        for (int j = i + 1; j < nodesNames.size(); j++)
        {
            ...
            {
                bool notDrawn = true;
                for (int m = 0; m < connectionsToDraw.size(); m++)
                {
...
void graph::placingStatesSlow(int hammingWeight)
{
   int size = std::pow(2, int(std::log2(nodesNames.size())) + 1);//TODO   
   ...
   std::vector<int> min_permutation;
   ...
   for (int i = 0; i < min_permutation.size(); i++)
   {
...
void graph::placingStates(int hammingWeight, std::vector<int> normesNumber)
{
    const int degree = std::ceil(std::log2(nodesNames.size()));
    const int size = std::pow(2, degree);
    int* permutation = new int[size];
    int* min_permutation = new int[size];
    for (int i = 0; i < size; i++)
        permutation[i] = i;
    int** rules = new int*[size];
    for (int i = 0; i < size; i++)
        rules[i] = new int[size];
    ...
    {
        int firstIdx = find(nodesNames.begin(), nodesNames.end(), connections[i].left) - nodesNames.begin();
        int secondIdx = find(nodesNames.begin(), nodesNames.end(), connections[i].right) - nodesNames.begin();
    ...
    int minNorm = std::numeric_limits<int>::max();
    std::vector<int> currentNormesNumber(normesNumber.size(), 0);
    do
    {
        ...
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                ...
                //{
                    for (int l = 0; l < rules[permutation[i]][permutation[j]]; l++)
                    {
                        int bits = (i^j);
                        int currentWeight = 0;
        ...
        if (currentNorm < minNorm)
        {
            ...
            cout << " Normes: ";
            for (int i = 1; i < currentNormesNumber.size(); i++)
            ...
            if (minNorm <= hammingWeight)
            {
                ...
                for (int i = 0; i < normesNumber.size(); i++)
...
void graph::placingStates(std::vector<int> permutation)
{
    nonterminalLettersContainer newNodesNames;
    for (int i = 0; i < permutation.size(); i++)
    {
...
void graph::determinancy()
{
    ...
    for (int j = 0; j < wasteNodes.size(); j++)
    {
        for (int i = 0; i < connections.size(); i++)
        {                    
    ...
    do
    {
        ...
        for (int i = 0; i < table.size(); i++)
        {
            for (int j = 0; j < table[i].size(); j++)
            {
                ...
                if (table[i][j] == equal)
                    continue;
                for (int k = 0; k < table[i][j].size(); k += 2)
                {
...
graph minimizationTable::minimizedGraph(graph oldGraph, nonterminalLetter newLetter)
{
    ...
    {
        std::string temp = "";
        for (int j = 0; j < equalClasses[i].size(); j++)
        {
    ...
    do
    {
        ...
        for (int i = 0; i < equalClasses.size(); i++)
        {
            for (int j = i + 1; j < equalClasses.size(); j++)
            {
    ...
    for (int i = 0; i < oldGraph.nodesNames.size(); i++)
    {
        bool contains = false;
        for (int j = 0; j < equalClasses.size(); j++)
        {
    ...
    for (int i = 0; i < oldGraph.specialStates.size(); i++)
    {
        for (int j = 0; j < equalClasses.size(); j++)
        {
    ...
    for (int i = 0; i < oldGraph.connections.size(); i++)
    {
...
triggerTable::triggerTable(graph cubedGraph)
{
    ...
    int size = std::log2(symbols.size());
    jumpConditionsNumbers.resize(symbols.size());
    for (int i = 0; i < jumpConditionsNumbers.size(); i++)
    {
        jumpConditionsNumbers[i].resize(size);
        for (int j = 0; j < jumpConditionsNumbers[i].size(); j++)
            jumpConditionsNumbers[i][j] = -4;
    }
    for (int i = 0; i < jumpConditionsNumbers.size(); i++)
    {
        bool foundInRules = false;
        for (int j = 0; j < cubedGraph.connections.size(); j++)
        {
            if (cubedGraph.connections[j].left == symbols[i])
            {
                foundInRules = true; 
                for (int k = 0; k < jumpConditionsNumbers[i].size(); k++)
                {
                    int left = (i >> k) & 1;
...
triggerTable::triggerTable(const triggerTable& other, int input)
{
    ...
    int size = std::log2(symbols.size());
    jumpConditions = other.jumpConditions;
    for (int i = 0; i < other.jumpConditionsNumbers.size(); i++)
    {
        ...
        for (int j = 0; j < jumpConditionsNumbers[i].size(); j++)
        {
            ...
            for (int k = 0; k < triggerFromDTable.size(); k++)
            {
...
std::vector<std::vector<int>> triggerTable::combinations(int n, int r)
{
...
void triggerTable::findСoating(std::vector<int> realNodes)
{
    ...
    {
        std::vector<std::vector<int>> comb = combinations(size, r);
        std::vector<std::vector<int>> candidats;
...
void triggerTable::minimizationBad()
{
    ...
    for (int k = 0; k < size; k++)
    {
        ...
        do
        {
            ...
            for (int i = indices[0]+1; i < sum.size(); i++)
            {
...
{    
    int edge = 130;
    int xDiagEdge = std::sqrt(edge*edge * 3) / 5;
    int yDiagEdge = std::sqrt(edge*edge * 3) / 4;
